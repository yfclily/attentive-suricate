...
When do you conduct an ANOVA? You conduct an ANOVA when you are testing the effect of one or more nominal (aka **factor**) **independent** variable(s) on a numerical **dependent** variable. A nominal (factor) variable is one that contains a finite number of categories with no inherent order.

If you only include one independent variable, this is called a **One-way ANOVA**. If you include two independent variables, this is called a **Two-way ANOVA**. If you include three independent variables, etc. 


`poopdeck` dataset: a dataframe containing the amount of time it took to clean both `parrot` and `shark` poop from the deck using three different cleaning solutions `a`, `b`, `c - Package `yarrr`,  Phillips (2018). 
...
Does the effect of cleaner depend on the poop type?|Two-way ANOVA with *interaction* term|`time ~ cleaner * type`
...
1. Create an ANOVA object using the `aov` function. In the `aov` function, specify the independent and dependent variable(s) with a formula with the format `y ~ x1 + x2` where y is the dependent variable, and x1, x2 … are one (more more) factor independent variables. The function `aov` combines a `lm` and `anova` functions together. 
...
We’ll set cleaning `time` as the dependent variable and the `cleaner` type  as the independent variable. We can represent the data as follow:
...
The main result from our table is that we have a significant effect of cleaner on cleaning time F(2, 597) = 5.29, p = 0.005. Carefuly examine the Df.

However, the ANOVA table does not tell us which levels of the independent variable differ. In other words, we don’t know which cleaner is better than which. To answer this, we need to conduct a **post-hoc** test.
...
To conduct a two-way ANOVA or more, just include additional independent variables in the regression model formula with the `+` sign as we did ealier in our regression models. That’s it. All the steps are the same. Let’s conduct a two-way ANOVA with both cleaner and type as independent variables. To do this, we’ll set `formula = time ~ cleaner + type`.
...
You can see, it becomes quickly much more complicated to interpret. Think carefully if interactions are justified. 

The package `interaction` provides several functions that can help at looking at the coefficients and to interpret in a more easy way the results.
...
#### Testing you assumptions 

The `residuals` values are obviously available in your anova object. We previously saw how to examine graphically those residuals for the normality and homoscedasticity. Let's test them her:

```{r,  eval=T, warning=F, message=F}
shapiro.test(cleaner.type.int.aov$residuals)
bartlett.test(cleaner.type.int.aov$residuals ~ interaction(cleaner, type), data = poopdeck)
```

The null hypothesis of normal distribution and homoscedasticity are accepted: there is no significant difference (p > 0.05) from a normal distribution and significant heterogeneity
...
This topic is extensively cover in online documentation in order for your model to follwo the assumption of normality and homescedasticity. In addition, that may be because your data are from a Gaussian family.
...
General linear model met all the assumptions: normal distribution, homoscedasticity, etc. Howvere, quite often in ecology and environmental science that is not the case and then we use different data distributions. Here we will talk about a Poisson and a binomial distribution. To use them, we need to run generalised linear models.

### A model with a Poisson distribution

Import the `shagLPI.csv` dataset and check it’s summary using `summary(shagLPI)`. The data represent population trends for European Shags on the Isle of May and are available from the [Living Planet Index](https://livingplanetindex.org/home/index).
...
## Taking Account the Random Effects - Mixed effects

A random effect can be used in the case of paired data or repeated measures. As an example, if we are measuring the length of several individual plants, the measurements are paired within each individual.  That is, we want to statistically match the length at different days of one individual, since we supposed we may have difference in the initial length and therefor different growth. Therefore, in this case, the variable 'Individual' must be include in the model as a random variable. Often, we want to disregard the effect of this random fectore (e.g. individual difference) to focus on another one (e.g. growth itself). Each Individual could be thought of has a block including a measurement for the length.
...
We want to examine the length of the plant (y) as a function of the day (x). In this case, the initial size of the plant varies. It become obvious that each teh growth of each pant should be examine individually.
...
In addition,  The `lmerTest` package is used to produce an analysis of variance with p-values for model effects. However, in our case, a simple linear regression with a random factor, we will use another function to extract teh p-value from our model.

#### Mixed model with `nmle`

We will use the function `lme` form the package `nmle`

Notice the grammar in the `lme` function that defines the model is slightly different form the previous exempl:  the option `random = ~1|ID` is added to the model to indicate that `ID` is the random term.
...
the "~1" specifies the random intercept and "Day|ID" would allow the slope to vary among  individuals. However,it starts to be much more advanced parametrization. 
