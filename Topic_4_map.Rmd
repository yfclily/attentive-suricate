...
\BATHYMETRY
<span style="color: #e1d9d7">Bathymetry data using the package  <span style="color: #a2c6e5">`marmap`</span></span>

```{r,  eval=FALSE}
# download data
TW.bathy <- getNOAA.bathy(lon1=118,lon2=124, lat1=21,lat2=26,resolution=0.5) # don't put too wide / resolution: 0.5 
# define palette
...
<span style="color: #e1d9d7">Using <span style="color: #a2c6e5">`dismo`</span> package for species distribution</span>

```{r,  eval=FALSE}
distri <- gbif("Acropora","muricata")
locs <- subset(distri, select = c("country", "lat", "lon"))
locs <- subset(distri, locs$lat < 90)
coordinates(locs) <- c("lon", "lat")  # set spatial coordinates
plot(locs)
crs.geo <- CRS("+proj=longlat +ellps=WGS84 +datum=WGS84")  # geographical, datum WGS84
proj4string(locs) <- crs.geo  # define projection system of our data
summary(locs)
plot(locs, axes=T,pch = 20, col = "red")
data(coastsCoarse)
data(countriesLow)
...
####***<span style="color: green">Exercise 5A:</span>***

#### *<span style="color: green">Using the packages of your choice:</span>*

#### *<span style="color: green">- make a map of the island for which you downloaded data: land in grey, sea in blue, add the important cities, the name of the island, the scale and the North arrow</span>*

#### *<span style="color: green">- in another figure, plot the bathymetry around the island together with its topography. Select a transect transversal to the island and plot the corresponding profile - add name of the island and seas. Plot both the bathymetry map and profile in one panel ! </span>*